...
import { InformationCircleIcon, FlagIcon,AtSymbolIcon ,BookOpenIcon } from '@heroicons/react/20/solid'
import {
  ArrowPathIcon,
  ChartPieIcon,
  CursorArrowRaysIcon,
...
const solutions = [
  {name: 'Inicio', description: 'pagina principal', href: '/', icon: FlagIcon },
  { name: 'Nosotros', description: 'información sobre C.A.Tac.Ṕol ', href: '#about', icon: InformationCircleIcon },
  { name: 'Biblioteca', description: 'libros', href: '/libros', icon: BookOpenIcon },
  { name: 'Contacto', description: 'puedes enviar tus consultas a esta dirreccion', href: '#contact', icon: AtSymbolIcon},
...
<Popover className="fixed z-50 w-full flex navbar justify-center ">
      <Popover.Button className="inline-flex items-center  bg-white-50 text-sm font-semibold bg-slate-50  text-gray-900">
      <div
              className="w-10  h-6   z-50 "
              onClick={handleMenu}
            ...
            >
              <div className="grid border  grid-navbar w-full h-full grid-cols-5 grid-rows-3">
                ...
                <div className="col-span-5  bg-sky-300">
                ...
                <div className="col-span-1 rounded-full overflow-hidden  bg-yellow-400">
      ...
      >
        <Popover.Panel className="absolute left-1/2 z-10 mt-5 flex w-screen max-w-max -translate-x-1/2 px-4 ">
          <div className="popover w-screen mt-6 max-w-md flex-auto overflow-hidden rounded-md bg-white text-sm leading-6 shadow-lg ring-1  ring-gray-900/5">
            ...
            {solutions.map((item) => (
                <div key={item.name} className="group relative flex gap-x-6 rounded-lg p-4  hover:bg-gray-800">
                  <div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg  group-hover:bg-white">
                    <item.icon className="h-6 w-6 text-white-50 group-hover:text-sky-400" aria-hidden="true" />
                  </div>
                  <div>
                    <Link href={item.href} className="font-semibold text-white-200">
                      {item.name}
